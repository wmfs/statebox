...
class Dao {
  ...
  describeExecution (executionName, updateLastDescribed) {
    const updateFn = updateLastDescribed === true
      ? execution => { execution.lastDescribed = new Date() }
      : () => {}

    return this._updateExecution(
      executionName,
      updateFn,
      boom.badRequest(`Unable to describe execution with name '${executionName}'`)
    )
  }
  ...
  failExecution (executionName, errorInfo) {
