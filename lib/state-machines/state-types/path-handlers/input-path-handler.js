...
function parameterHandler (parameters) {
  ...
  return replacements.length ? makeDynamicHandler(params, replacements) : makeFixedHandler(params)
} // parameterHandler
...
function makeDynamicHandler (params, references) {
  ...
  return ctx => {
    ...
    replacers.forEach(([path, expr]) => {
      const value = _.cloneDeep(jp.value(ctx, expr))
      dottie.set(parameters, path, value)
    })
    return parameters
  }
} // makeDynamicHandler
