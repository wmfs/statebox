  ...
  return ctx => {
    ...
    for (const [path, expr] of replacers) {
      ...
      if (isJSONPath(expr)) {
        ...
        extractedValue = extractValue(values)
      } else if (isIntrinsicFn(expr)) {
...
function isIntrinsicFn (p) {
  return _.isString(p) && p.length !== 0 && (
    p.startsWith('States.Array') ||
    p.startsWith('States.Format') ||
    p.startsWith('States.StringToJson') ||
    p.startsWith('States.JsonToString')
  )
}
