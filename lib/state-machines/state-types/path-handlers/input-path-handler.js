...
function makeDynamicHandler (params, references) {
  ...
  return input => {
    ...
    for (const [path, expr] of replacers) {
      ...
      if (isJSONPath(expr)) {
        extractedValue = evaluateArgument(expr, input)
      } else if (instrinsics.isFunctionCall(expr)) {
        const compiled = _.template(`<% return ${expr} %>`)
        extractedValue = compiled({
          $: input,
          States: {
            Array: (...elements) => elements,
            Format: instrinsics.format,
            StringToJson: str => JSON.parse(str),
...
const Evaluate = {
  path: (expr, input) => extractValue(_.cloneDeep(jp.query(input, expr))),
} // Evaluate

function evaluateArgument (argString, input) {
  const token = argTokeniser(argString)
  return Evaluate[token.type](token.value, input)
} // evaluateArgument
